...
export const buildScenarioRng = (scenarioId) => seedrandom(`scenario-${scenarioId}`);

export const generateSimReading = ({
  scenarioId,
  prevReading,
  tick,
  rng,
  ...
  const prev = prevReading || createReading({
    co2: base.co2,
    pm25: base.pm25,
  });
  ...
  const co2Delta = randomBetween(drift.co2[0], drift.co2[1], rng) + sinusoidal(tick, 12);
  const pmDelta = randomBetween(drift.pm25[0], drift.pm25[1], rng) + sinusoidal(tick, 18) * 0.2;
